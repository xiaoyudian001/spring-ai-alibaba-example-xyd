...
- Java 源码文件必须使用 `UTF-8` 无 BOM 编码。
- 新增或修改 `minimax-chat` 下的 Java 文件时，不要写入 UTF-8 BOM，否则 Maven 编译可能出现 `非法字符: '\ufeff'`。
- 如果编译日志出现 `非法字符: '\ufeff'`，优先按文件编码问题处理，不要误判为业务代码语法错误。
- 批量修复 BOM 时，可以将相关 Java 文件统一转换为 UTF-8 无 BOM，但不要改动文件业务内容。
- 修复 BOM 后必须重新运行相关模块编译，确认后续暴露的才是真实 API 或业务代码问题。
